...
void RTSSHOW::RTS_HandleData()
{
  ...
  switch(Checkkey)
  {
    ...
    case Zoffset:
        ...
        {
          zprobe_zoffset = ((float)recdat.data[0]-65536)/10;
        }
        else
        {
          zprobe_zoffset = ((float)recdat.data[0])/10;
        }
        if (WITHIN((zprobe_zoffset), Z_PROBE_OFFSET_RANGE_MIN, Z_PROBE_OFFSET_RANGE_MAX)) 
        {
			babystep.add_steps(Z_AXIS, (BABYSTEP_MULTIPLICATOR_Z) * (zprobe_zoffset - last_zoffset));
        }
      #endif
      break;
...
void EachMomentUpdate()
{
  ...
  {
    if ((power_off_type_yes == 0)  && LCD_sd_status && power_loss_status > 0)
	{
      if(startprogress == 0)
      {
      ...
      if((startprogress +=1) > 100)
      {
        ...
        for (uint16_t i = 0; i < CardRecbuf.Filesum ; i++) 
        {
		  SERIAL_ECHOLN(CardRecbuf.Cardfilename[i]);
		  SERIAL_ECHOLN(recovery.info.sd_filename);
	...
	else if((power_off_type_yes == 0) && !power_loss_status)
	{
		if(startprogress == 0)
		{
		  rtscheck.RTS_SndData(StartSoundSet, SoundAddr);
